...
const orderCheckout = async (req, res) => {
  try {
    let UserExist = req.session.user.id ? true : false;
    console.log(req.session.user.id);
    const addresses = await addressModel.findOne({ user: req.session.user.id });
    // const userDetail = await userModel.findOne({ _id: req.session.userId });
    const category = await CategoryModel.find();
    const cartCheckout = await cartModel.findOne({ owner: req.session.user.id });
...
let orderCheckoutPost = async (req, res, next) => {
  try {
    ...
    const cart = await cartModel.findOne({ owner: req.session.user.id });
    ...
    const Address = await addressModel.findOne({ user: req.session.user.id });
    ...
    if (req.body.paymentOption === "cashOnDelivery") {
      ...
      const options = {
        amount,
        currency: "INR",
        receipt: "anaghakb55@gmail.com", // Replace with your email
      };
...
const orderConfirmation = async (req, res) => {
  ...
  try {
    ...
    await cartModel.findOneAndUpdate({ owner: req.session.user.id }, {
      $set: {
        items: []
      }
    })
