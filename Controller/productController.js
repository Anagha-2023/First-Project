...
const productManagementGet = async (req, res) => {
    try {
        ...
        console.error('Error in productManagementGet:', error);
        res.status(500).json({
    ...
    try {
        ...
        if (countInStock < 0 || price < 0 || discountPrice < 0) {
            return res.status(400).json({
                error: 'Invalid input values. Please provide non-negative values for Count In Stock, Original price, and Discount Price.'
            });
        }
        ...
        images.forEach((item, index) => {
            let isThere = existingProduct.images.find((image) => image !== item);
            if (!isThere) {
                log('work aaye');
